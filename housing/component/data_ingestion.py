...
class DataIngestion:
    ...
    def download_housing_data(self):
        try:
            #extracting the remote URL to download the dataset
    ...
    def extract_tgz_file(self, tgz_file_path:str):
        try:
            ...
            logging.info(f"Extraction tgz file [{tgz_file_path}] completed : [{raw_data_dir}]")
        except Exception as e:
            raise HousingException(e,sys) from e
    ...
    def split_data_as_train_test(self) -> DataIngestionArtifact:
        try:
            ...
            housing_data_frame['income_cat'] = pd.cut(
                housing_data_frame['median_income'], bins=[0,1.5,3.0,4.5,6.0,np.inf], labels=[1,2,3,4,5]
            )
            ...
            if strat_train_set is not None:
                os.makedirs(self.data_ingestion_config.train_data_dir, exist_ok=True)
                strat_train_set.to_csv(train_file_path, index=False)

            if strat_test_set is not None:
                os.makedirs(self.data_ingestion_config.test_Data_Dir, exist_ok=True)
